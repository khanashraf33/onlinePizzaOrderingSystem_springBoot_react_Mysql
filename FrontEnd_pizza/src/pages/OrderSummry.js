...
const OrderSummry = () => {
  ...
  const loadCart= async ()=>{
    ...
    await axios.get(`http://13.233.168.239:8080/pizzadelivery/cart/${user.id}`).then((res)=>{
        setCart(res.data)
  ...
  const loadItems= async ()=>{
      await axios
      .get(`http://13.233.168.239:8080/pizzadelivery/cartitems/${cart.id}`)
      .then((res)=>{
    ...
    const placeOrder=async()=>{
        try{
          await axios.post('http://13.233.168.239:8080/pizzadelivery/checkout',{
            cartOwner: user,
            address: address,
