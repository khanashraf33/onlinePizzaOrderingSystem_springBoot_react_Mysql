...
const OrderDetails = () => {
    ...
    //   await axios.get(`http://13.233.168.239:8080/pizzadelivery/order/${id}`).then((res)=>{
    ...
    const loadCart = async () => {
      try {
        const response = await axios.get(`http://13.233.168.239:8080/pizzadelivery/order/${id}`);
        const data = response.data;
        setAddress(data.address);
        setUser(data.cartOwner);
        setOrder(data);
        setPaymentMethod(data.paymentType);
        setTotalPrice(data.totalOrderPrice);
        setCartPrice(data.cartPrice);
        setShippingPrice(data.deliveryPrice);
        loadItems();
      } catch (error) {
    ...
    const loadItems= async ()=>{
        await axios.get(`http://13.233.168.239:8080/pizzadelivery/orderitemsbyorder/${id}`).then((res)=>{
