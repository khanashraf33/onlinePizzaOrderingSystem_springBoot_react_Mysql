...
const CartItemCard = ({item},{onIncreament}) => {
    ...
    const increaseQty= async()=>{
        await axios.get(`http://13.233.168.239:8080/pizzadelivery/addtocart?user_id=${userId}&pizza_id=${item.pizza.id}`)
        // handleClick()
    ...
    const decreaseQty=async()=>{
        await axios.get(`http://13.233.168.239:8080/pizzadelivery/removefromcart?user_id=${userId}&pizza_id=${item.pizza.id}`)
        // handleClick()
        window.location.reload()
    }
    const deletecartitem=async()=>{
        await axios.delete(`http://13.233.168.239:8080/pizzadelivery/cartitems/${item.id}`)
        // handleClick();
        window.location.reload()
    }
